...
export const TopNav: React.FC<TopNavProps> = ({ onToggleSidebar, onResetLayout }) => {
  ...
  return (
    <header className="h-16 flex-shrink-0 bg-white dark:bg-cyber-card border-b border-slate-200 dark:border-cyber-700/60 px-4 sm:px-6 flex items-center justify-between z-30 transition-colors">
      {/* Left: Mobile Sidebar Toggle + Navigation / Mode */}
      <div className="flex items-center space-x-3">
        <button
          onClick={onToggleSidebar}
          className="p-1.5 rounded-sm text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-cyber-700/60 transition focus:outline-none"
          title="Toggle Navigation Menu"
        >
          <Menu className="w-5 h-5" />
        </button>

        <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider hidden sm:block">
        ...
        <div className="hidden md:flex items-center border border-slate-200 dark:border-cyber-700/70 rounded-sm divide-x divide-slate-200 dark:divide-cyber-700/70 overflow-hidden text-[10px] font-mono">
          <span className="px-2.5 py-1 text-slate-700 dark:text-slate-300 font-semibold flex items-center space-x-1.5">
          ...
          {isThreatsPage ? (
            <Link
              to="/dashboard"
              className="px-2.5 py-1 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-cyber-700/60 transition flex items-center space-x-1"
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex items-center border border-slate-200 dark:border-cyber-700/70 rounded-sm divide-x divide-slate-200 dark:divide-cyber-700/70 overflow-hidden text-xs">
          <button
            onClick={openReport}
            className="px-3.5 py-1.5 flex items-center space-x-1.5 font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-cyber-700/60 transition"
          >
          ...
          <button
            onClick={toggleAudit}
            className="px-3.5 py-1.5 font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-cyber-700/60 transition"
          >
            <span>Audit</span>
          </button>
        </div>
        ...
        {/* Vertical Divider */}
        <div className="h-6 w-px bg-slate-200 dark:bg-cyber-700/70"></div>

        {/* User Profile Dropdown Button & Menu */}
        <div className="relative" ref={profileRef}>
          <button
            onClick={() => setProfileOpen(!profileOpen)}
            className="flex items-center space-x-2 p-1 pr-2 rounded-sm hover:bg-slate-100 dark:hover:bg-cyber-700/60 transition focus:outline-none"
          >

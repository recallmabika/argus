...
export const DevicesTableCard: React.FC<DevicesTableCardProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-cyber-card rounded-sm p-4 flex flex-col shadow-xs h-full" id="devices">
        ...
        <div className="flex items-center space-x-2">
          {onPullToMain && (
            <button
              ...
              className={`px-2 py-0.5 rounded-sm text-[10px] font-mono transition flex items-center space-x-1 cursor-pointer shadow-xs ${
                isMainStage
      ...
      <div className="flex-1 overflow-x-auto custom-scrollbar pt-1">
        <table className="w-full text-left font-mono text-xs">
          <thead className="bg-slate-50/80 dark:bg-cyber-800/40 text-[10px] uppercase text-slate-400 tracking-wider rounded-sm">
            <tr>
              <th className="py-2.5 px-3 rounded-l-sm">Hostname</th>
              <th className="py-2.5 px-3">OS</th>
              <th className="py-2.5 px-3">User</th>
              <th className="py-2.5 px-3">Branch</th>
              <th className="py-2.5 px-3">Risk</th>
              <th className="py-2.5 px-3">Status</th>
              <th className="py-2.5 px-3 text-right rounded-r-sm">Action</th>
            </tr>
          </thead>
          <tbody className="font-mono text-xs">
            {loading ? (
              ...
              devices.map((d) => (
                <tr key={d.id} className="hover:bg-slate-50/70 dark:hover:bg-cyber-800/40 transition">
                  <td className="py-2.5 px-3 font-semibold text-slate-900 dark:text-white rounded-l-sm">{d.hostname}</td>

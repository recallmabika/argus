...
export const DevicesTableCard: React.FC<DevicesTableCardProps> = ({ devices, loading = false }) => {
  ...
  return (
    <div className="bg-white dark:bg-cyber-card border border-slate-200 dark:border-cyber-700/60 rounded-sm p-4 flex flex-col shadow-xs" id="devices">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-cyber-700/50">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">
            Monitored Endpoint Fleet ({devices.length})
          </h3>
        </div>
        <span className="text-[10px] font-mono text-slate-400">Agent Telemetry</span>
      </div>
